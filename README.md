# WATS Converter - Seica XML format

A WATS Client converter plugin for importing test data from Seica test system to WATS.
...
This is an example converter for Seica that only supports output in the same format as the example files. Different configurations of the test software will require an accordingly different converter.
...
* In TestSeicaXMLConverter, fill in the path to the file you want to test the converter with. There are example files in the Data folder.
* Run SetupClient once, then you can run TestSeicaXMLConverter as many times as you want.
